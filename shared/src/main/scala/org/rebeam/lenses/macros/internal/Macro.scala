package org.rebeam.lenses.macros.internal

import monocle._
import org.rebeam.lenses.PLensN

import scala.reflect.macros.blackbox
import scala.language.experimental.macros

object Macro {
  def mkLens[S, T, A, B](fieldName: String): PLensN[S, T, A, B] = macro MacroImpl.mkLens_impl[S, T, A, B]
}
...
private[macros] class MacroImpl(val c: blackbox.Context) {
  ...
  def mkLens_impl[S: c.WeakTypeTag, T: c.WeakTypeTag, A: c.WeakTypeTag, B: c.WeakTypeTag](fieldName: c.Expr[String]): c.Expr[PLensN[S, T, A, B]] = {
    import c.universe._
    ...
    c.Expr[PLensN[S, T, A, B]](q"""
      import org.rebeam.lenses.PLensN
      import scalaz.Functor

      new PLensN[$sTpe, $tTpe, $aTpe, $bTpe]{
        def get(s: $sTpe): $aTpe =
          s.$fieldMethod
